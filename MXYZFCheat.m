...
#pragma mark - 悬浮球 view（彩虹环 + 圆头像，view 复用工厂）
// 彩虹环球：外层渐变圆挖空成环 + 内部圆头像。整个 view 大小 = size。
static UIView *mx_ball_view(CGFloat size) {
    ...
    wrap.userInteractionEnabled = NO;   // 手势在宿主 window 的根 view 上处理

    // 彩虹渐变环
    CAGradientLayer *grad = [CAGradientLayer layer];
    grad.frame = wrap.bounds;
    grad.type = kCAGradientLayerConic;   // 圆锥渐变 = 抖音同款环形彩虹
    grad.colors = @[
        (id)[UIColor colorWithRed:0.10 green:0.90 blue:1.00 alpha:1].CGColor, // 青
        ...
        (id)[UIColor colorWithRed:0.10 green:0.90 blue:1.00 alpha:1].CGColor, // 青（闭环）
    ];
...
#pragma mark - 独立 window 宿主
#define BALL_SIZE 58.0
static UIWindow   *g_win    = nil;   // 独立悬浮 window
static UIView     *g_ball   = nil;   // 球 view（在 g_win.rootViewController.view 上）
static UIView     *g_panel  = nil;   // 面板 view（同 window）

// 穿透 view：非球/非面板区域把触摸交回下层 window
@interface MXPassthroughView : UIView
@end
@implementation MXPassthroughView
- (UIView *)hitTest:(CGPoint)pt withEvent:(UIEvent *)event {
    UIView *hit = [super hitTest:pt withEvent:event];
    // 命中自己（空白区）→ 返回 nil 让事件穿透到下层游戏 window
...
- (void)togglePanel {
    ...
    // 面板出现在球旁边（自动避屏边）
    CGFloat px = bx + BALL_SIZE/2 + 8;
    CGFloat py = by - ph/2.0;
    ...
    g_panel.backgroundColor = mx_c(16, 18, 26, 0.97);
    g_panel.layer.borderColor = mx_c(70, 140, 255, 0.45).CGColor;
    g_panel.layer.borderWidth = 1;
    g_panel.userInteractionEnabled = YES;

    UIView *av = mx_ball_view(40);
    ...
    UILabel *sub = [[UILabel alloc] initWithFrame:CGRectMake(14, 66, pw-28, 60)];
...
static void mx_build_window(void) {
    ...
    g_ball.layer.shadowOpacity = 0.4;
    g_ball.layer.shadowRadius = 4;
    g_ball.layer.shadowOffset = CGSizeMake(0, 2);
    [root addSubview:g_ball];

    // 手势：单点 tap + 拖动（都在球 view 上）
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:[MXBox shared] action:@selector(ballTap)];
    [g_ball addGestureRecognizer:tap];
    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:[MXBox shared] action:@selector(ballDrag:)];
    [g_ball addGestureRecognizer:pan];

    mlog(@"overlay window built (level=%g)", g_win.windowLevel);
}

// 保活 tick：游戏可能释放/重建 window，球没了就重建
static NSTimer *g_keep = NULL;
static void mx_keepalive_tick(void) {
    @autoreleasepool {
        @try {
            ...
            // 确保在最顶（游戏也可能加同 level 的 window）
            if (g_win && g_win.windowLevel < CGFLOAT_MAX) g_win.windowLevel = CGFLOAT_MAX;
        } @catch (NSException *e) {
            mlog(@"keepalive exc: %@", e.name);
        }
    }
}
...
__attribute__((constructor)) static void mxzf_ctor(void) {
    @autoreleasepool {
        ...
        mlog(@"ctor pid=%d bid=%@ (pure-UI v2)", getpid(), bid);
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            mx_build_window();
            g_keep = [NSTimer timerWithTimeInterval:2.0 target:[MXBox shared] selector:@selector(keepTick) userInfo:nil repeats:YES];
            [[NSRunLoop mainRunLoop] addTimer:g_keep forMode:NSRunLoopCommonModes];
        });
    }
}
@implementation MXBox (Keep)
- (void)keepTick { mx_keepalive_tick(); }
@end
